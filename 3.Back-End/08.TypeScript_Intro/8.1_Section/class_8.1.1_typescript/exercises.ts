...
export function getRhombusArea(length: number, height: number): number {
  return (length * height) / 2;
}

export function getTrapezoidArea(base1: number, base2: number, height: number): number {
  return (base1 + base2) * height / 2;
}

export function getCircleArea(radius: number): number {
  const formula: number = Math.PI * radius ** 2;
